    ...
    <StyledNav>
      <NavContainer>
        <LogoContainer as={Headers} level={1}>
          <span>
            Media
            <FontAwesomeIcon icon="arrows-left-right-to-line" />
          </span>
          Amator
        </LogoContainer>

        <NavbarButtons>
          <LoginButton link="/login">{!loading ? showAccountOption() : ". . ."}</LoginButton>
        ...
        <BurgerMenu>Menu{cart.quantity > 0 && <span>: {cart.quantity}</span>}</BurgerMenu>
      </NavContainer>
    </StyledNav>
  );
};
